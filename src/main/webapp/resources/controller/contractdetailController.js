app.controller('contractdetailController',function($scope,$timeout, $localStorage, $http, $interval){
	...
	$scope.getStatusChat=function(code,msg){
		...
		if(code == "DA_CD_KIND_DECRYPT_KEY"){
			if($scope.isencrypted=='Yes'){
				$scope.showDecryptBtn = true;
				return "";
				//return statusMsg +" "+msg;
			}else{
				$scope.showDecryptBtn = false;
				return statusMsg +" "+msg;
			}
		}else{
			$scope.showDecryptBtn = false;
					...
					data : postData
						...
						if (response.data){
							...
							for (var i=0; i<$scope.contractHistoryData.length; i++){
								...
								if ($scope.contractHistoryData[i].Value.statuscode == "DA_CST_API_EXCH" && $scope.contractHistoryData[i].Value.messagedp == "DA_CD_KIND_DECRYPT_KEY"){
									//if ($scope.contractHistoryData[i].Value.statuscode == "DA_CD_KIND_DECRYPT_KEY"){
									
									$scope.endpointMsg = $scope.contractHistoryData[i].Value.endpointurl;
									$scope.successMessage = $scope.contractHistoryData[i].Value.message;
									$scope.statuscode = $scope.contractHistoryData[i].Value.statuscode;
									
									$scope.isencrypted = $scope.contractHistoryData[i].Value.isencrypted;
									if($scope.isencrypted =='Yes'){
										$scope.showDecryptBtn = true;
										$scope.apidocument = $scope.contractHistoryData[i].Value.apidocument;
										$scope.endpointexpiry = $scope.contractHistoryData[i].Value.endpointexpiry;
									}else{
										$scope.showDecryptBtn = false;
										$scope.apidocument = '';
										$scope.endpointexpiry = '';
									}
		...
		$http({
			...
			data : postData
				...
				if (response.data){
					...
					$scope.loadeer = false;						
					$scope.newdesc = "";
					$scope.issendbtnsubmitted=false;
					$scope.sendButtonStatus = false;
					$scope.refreshContractData();
					$scope.getContractHistory();
					$scope.showspinner = false;
...
//				$scope.showPayProc = true;
	...
	$scope.decryptMdl = function(){
		jQuery('#decryptModal').modal('show');
	}
	
	$scope.decryptresMessage='';
	$scope.decrypterrMessage='';
	$scope.privateKey ='';
	$scope.isbuttonsubmitted = false;
	$scope.decryptKey = function() {
	$scope.isbuttonsubmitted = true;
		
		$http({
			url : RSA_DECODE,
			method : 'POST',
			headers : {
		        'x-api-key': "OaYpCkgRDn1DBbh4HGO4m4fKaX4RrX82tqepCb3d"
		    },
			contentType : 'application/json',
			data : {"encodeText" : $scope.endpointMsg, "key" :$scope.privateKey}
			}).then(function(response) {
				$scope.isbuttonsubmitted = false;
				console.log(response);
				if(response.data.result!=''){
					$scope.decryptendpointurl = response.data.result;
					jQuery('#decryptModal').modal('hide');
					jQuery('#decryptsuccessModal').modal('show');
					$scope.decryptresMessage = $scope.successMessage;
					
				}else{
					$scope.decrypterrMessage = 'Decryption unsuccessfull. Please check that the private key you entered corresponds to the public key above.';
				}				
		})
	}
	
	$scope.decryptmdlClear = function(){
		$scope.decryptresMessage='';
		$scope.decrypterrMessage='';
		$scope.privateKey ='';
	}
