...
//		jQuery('#spinnerpurchase').modal('show');
	...
	*/
	...
	$scope.checkIsEnoughBalance = function(data){
		...
		$http({
			method: "POST",
		    url: 'https://api.sentient.io/microservices/dev/da/spendingaccountinfo',
		    contentType : 'application/json',
		    headers : {
		        'x-api-key': API_USER_KEY
		    },
...
//		data.Record.spinnerForBuynow = true;
	...
	$scope.getMarketPlaceData = function() {
			...
			console.log($localStorage.expiryContracts);
			var a1 = ["4","177","216","2","7","3","1"].concat($localStorage.expiryContracts);
			//var jsonFilterData = {"selector": {"$nor": $localStorage.expiryContracts, "status":"active"},"skip": $scope.skipCount, "sort": [{"datecreated": "desc"}]};
//			var jsonFilterData = {"selector": {"$not": {"orgid":ORG_ID+""},"dataprovidername":"sentient io","status":"active"}, "sort": [{"datecreated": "desc"}]};
//			var jsonFilterData = {"selector": {"$not": {"orgid":ORG_ID+""},"status":"active"}, "sort": [{"datecreated": "desc"}]};
